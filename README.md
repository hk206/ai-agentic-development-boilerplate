...
スキル・エージェント・チームはフェーズと規模で使い分ける。

| 手段 | 使うとき |
|------|---------|
| **スキル直接** | やることが明確で単発のタスク |
| **エージェント** | 専門性が必要で、判断を含む連続作業 |
| **エージェントチーム** | 複数の観点が同時に必要、または並列で進められる作業 |

| フェーズ | おすすめ | 理由 |
|---------|---------|------|
| 企画 | チーム | PM + デザイナー + 技術で多角的に検討できる |
| 要件定義 | PMエージェント | PRD → ストーリー → タスク → Notion同期を文脈を保って一気通貫 |
| UI設計 | デザイナーエージェント or `/design-component` | コンポーネント単位なら直接スキルでも可 |
| 技術設計 | `/plan-architecture` | 独立コンテキストで実行され、メイン作業を中断しない |
| 実装 | 直接指示 or チーム | 大きな機能はモジュール分割してチームで並列実装 |
| レビュー | `/review-pr` or チーム | 重要PRはセキュリティ・パフォーマンス・テストを同時レビュー |
| バグ対応 | デバッガーエージェント or チーム | 原因不明の大きなバグはチームで複数仮説を並列検証 |
