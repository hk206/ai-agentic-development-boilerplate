...
スキル・エージェント・チームはフェーズと規模に応じて使い分ける。

### 使い分けの基本

| 手段 | 使うとき |
|------|---------|
| **スキル直接** | やることが明確で単発のタスク（例: `/write-tests src/lib/utils.ts`） |
| **エージェント** | 専門性が必要で、判断を含む連続作業（例: PMエージェントに要件整理を一任） |
| **エージェントチーム** | 複数の観点が同時に必要、または並列で進められる作業 |

### フェーズ別ガイド

#### 1. 企画フェーズ
- **チーム推奨**: PM（市場分析・ビジョン）+ デザイナー（UX視点）+ 技術（実現可能性）で多角的に検討
- 小規模なら: PMエージェント単体、または `/ideate-product` を直接使用

#### 2. 要件定義フェーズ
- **エージェント推奨**: PMエージェントが PRD → ストーリー → タスク分解 → Notion同期まで連続実行
- スキル直接でもよいが、エージェントの方が文脈を保ったまま一貫した成果物を出せる

#### 3. 設計フェーズ
- UI設計 → デザイナーエージェント or `/design-component`
- 技術設計 → `/plan-architecture`（独立コンテキストで実行されるため、メインの作業を中断しない）

#### 4. 実装フェーズ
- 基本は Claude Code に直接指示して実装する
- 大きな機能は**チーム**: モジュールごとに担当を分けて並列実装
- テストは `/write-tests` または QAエージェント

#### 5. レビューフェーズ
- 通常のPR: `/review-pr` で十分
- 重要なPR: **チーム**でセキュリティ・パフォーマンス・テストカバレッジを異なる観点で同時レビュー

#### 6. バグ対応
- 小さなバグ: デバッガーエージェント単体
- 原因不明の大きなバグ: **チーム**で複数仮説を並列検証
